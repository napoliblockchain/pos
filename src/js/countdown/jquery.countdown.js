 ...
 * @modified by Sergio Casizzone
 * @version 	1.1
 * @url			http://sergiocasizzone.it
 * @license		MIT License
 */
...
(function($){
	...
	$.fn.countdown = function(prop){
		...
		var fg = false;
		var tempo;
		var scadenza = Math.floor((options.timestamp) / 1000);
		...
		(function tick(){
			if (!(fg))
			{
				ServerTime = new Date;
				fg = true;
				tempo = Math.floor(ServerTime) / 1000;
			}
			else
			{
				tempo = eval(tempo +1);
			...
			if(left < 0){
				...
				if ($( "#invoiceValue" ).val() == 'new' || $( "#invoiceValue" ).val() == 'expired'){
					$('.return-button').show();
